...
const normalizeIntentText = (raw: any): string => {
    const s = String(raw ?? '')
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '');
...
const mergeUniqueTokens = (...groups: string[][]): string[] => {
    const out: string[] = [];
...
const buildLocalIntentExpansion = (query: string): string[] => {
    const q = normalizeIntentText(query);
    if (!q) return [];

    const expansions: string[] = [];
    const add = (...tokens: string[]) => expansions.push(...tokens);

    if (/oficina|trabajo|formal|elegante/.test(q)) add('limpio', 'suave', 'fresco', 'elegante', 'versatil');
    if (/noche|fiesta|cita|seductor|sensual/.test(q)) add('ambar', 'vainilla', 'intenso', 'seductor', 'nocturno');
    if (/calor|verano|dia|deporte|gym|gimnasio/.test(q)) add('citrico', 'acuatico', 'verde', 'refrescante', 'ligero');
...
export const refineSearchQuery = async (query: string): Promise<string[]> => {
    ...
    if (!trimmed.includes(' ')) return [normalizeIntentText(trimmed)];

    // Optimizacion 2: Cache de resultados previos
    const cacheKey = `${CACHE_KEYS.AI_REFINE}${normalizeIntentText(trimmed)}`;
    ...
    if (!GEMINI_API_KEY || !gemini) {
        ...
        if (fallbackOnly.length > 0) {
            appCache.set(cacheKey, fallbackOnly, 6 * 60 * 60 * 1000);
        }
        return fallbackOnly;
    }
...
- Responde ÚNICAMENTE con la lista separada por comas, sin etiquetas ni explicaciones.`;
        ...
        if (fallback.length > 0) {
            appCache.set(cacheKey, fallback, 6 * 60 * 60 * 1000);
        }
        return fallback;
    }
};
