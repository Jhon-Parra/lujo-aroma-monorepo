...
const sourcePath = path_1.default.resolve(process.cwd(), sourcePathArg);
...
const token = (() => {
    ...
    return jsonwebtoken_1.default.sign({ id: 'image-bot', email: 'bot@lujoyaroma.local', rol: 'SUPERADMIN', sub: 'image-bot', isLocal: true }, secret, { expiresIn: '8h' });
})();
...
const readSourceCsv = async (filePath) => {
...
const removeWhiteBackgroundToPng = async (input) => {
    ...
    for (let i = 0; i < data.length; i += 4) {
        ...
        if (white >= 248)
            ...
            data[i + 3] = Math.min(data[i + 3], Math.max(0, (248 - white) * 14));
    }
    return await (0, sharp_1.default)(data, {
        raw: { width: info.width, height: info.height, channels: info.channels }
    }).png({ compressionLevel: 9 }).toBuffer();
};
const updateProductImage = async (id, name, imageBuffer) => {
    ...
    for (let attempt = 1; attempt <= 4; attempt++) {
        ...
        if (attempt === 4)
            throw new Error(`Update failed (${res.status}): ${text.slice(0, 220)}`);
...
const run = async () => {
    ...
    const tasks = sourceRows.map((row) => {
        if (row.id && byId.has(row.id))
            return { row, product: byId.get(row.id) };
        const nm = normalize(String(row.nombre || ''));
        if (nm && byName.has(nm))
            return { row, product: byName.get(nm) };
        return null;
    }).filter(Boolean);
