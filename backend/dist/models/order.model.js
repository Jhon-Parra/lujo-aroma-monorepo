...
let _paymentCols = null;
const detectPaymentColumns = async () => {
    if (_paymentCols)
        return _paymentCols;
    try {
        const [rows] = await database_1.pool.query(`SELECT column_name FROM information_schema.columns
             WHERE table_schema = DATABASE()
               AND lower(table_name) = 'ordenes'
               AND column_name IN ('estado_pago','referencia_pago','fecha_pago')`);
        const cols = new Set((rows || []).map((r) => String(r.COLUMN_NAME || r.column_name || r.Column_Name).toLowerCase()));
        _paymentCols = {
...
const idToSql = async (uuid) => {
    const binary = await detectIdType();
    if (binary)
        return { expr: 'UUID_TO_BIN(?)', val: uuid };
    return { expr: '?', val: uuid };
};
const binToUuidExpr = async (field) => {
    const binary = await detectIdType();
...
const detectAddonConfigColumns = async () => {
    try {
        const [rows] = await database_1.pool.query(`SELECT COUNT(*) AS cnt FROM information_schema.columns
              WHERE table_schema = DATABASE()
...
const getAddonConfig = async () => {
    ...
    if (!supported)
        return { envio_prioritario_precio: 0, perfume_lujo_precio: 0, empaque_regalo_precio: 0, supported: false };
    try {
        // Detectar qué columnas de precio existen para no fallar si alguna falta
        const [existCols] = await database_1.pool.query(`SELECT column_name FROM information_schema.columns
             WHERE table_schema = DATABASE()
               AND lower(table_name) = 'configuracionglobal'
               AND column_name IN ('envio_prioritario_precio','perfume_lujo_precio','empaque_regalo_precio')`);
        const existSet = new Set((existCols || []).map((r) => String(r.COLUMN_NAME || r.column_name || '').toLowerCase()));
        const selectParts = [];
        if (existSet.has('envio_prioritario_precio'))
            selectParts.push('COALESCE(envio_prioritario_precio, 0) AS envio_prioritario_precio');
        else
            selectParts.push('0 AS envio_prioritario_precio');
        if (existSet.has('perfume_lujo_precio'))
            selectParts.push('COALESCE(perfume_lujo_precio, 0) AS perfume_lujo_precio');
        else
            selectParts.push('0 AS perfume_lujo_precio');
        if (existSet.has('empaque_regalo_precio'))
            selectParts.push('COALESCE(empaque_regalo_precio, 0) AS empaque_regalo_precio');
        else
            selectParts.push('0 AS empaque_regalo_precio');
...
const computeSubtotal = (items) => round2((items || []).reduce((sum, it) => {
...
class OrderModel {
    ...
    static isValidTransition(actual, nuevo) {
    ...
    static async getOrderStatus(orderId) {
        ...
        const [rows] = await database_1.pool.query(`SELECT estado FROM ordenes WHERE id = ${expr} LIMIT 1`, [val]);
    ...
    static async createOrder(orderData) {
        ...
        try {
            ...
            const total = round2(Math.max(0, subtotal_productos - cart_recovery_discount_amount) +
            ...
            const cols = ['id', 'usuario_id', 'total', 'direccion_envio', 'estado', 'codigo_transaccion', 'telefono', 'nombre_cliente', 'metodo_pago', 'canal_pago'];
            const vals = [
            ...
            if (addonCols.empaque_regalo) {
                cols.push('empaque_regalo');
                vals.push(empaque_regalo);
            }
            if (addonCols.costo_empaque_regalo) {
                cols.push('costo_empaque_regalo');
                vals.push(costo_empaque_regalo);
            }
            if (addonCols.cart_recovery_applied) {
                cols.push('cart_recovery_applied');
                vals.push(cart_recovery_applied);
            }
            if (addonCols.cart_recovery_discount_pct) {
                cols.push('cart_recovery_discount_pct');
                vals.push(cart_recovery_discount_pct);
            }
            if (addonCols.cart_recovery_discount_amount) {
                cols.push('cart_recovery_discount_amount');
                vals.push(cart_recovery_discount_amount);
            }
            const placeholders = cols.map((c) => (c === 'id' || c === 'usuario_id') ? idExpr : '?').join(', ');
            ...
            for (const item of orderData.items) {
                ...
                await connection.query(binary
                    ...
                    : `INSERT INTO detalleordenes (id, orden_id, producto_id, cantidad, precio_unitario, nombre_producto, imagen_url, subtotal_snapshot)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [itemId, orderId, item.product_id, item.quantity, item.price, nombre_snapshot, imagen_snapshot, subtotal_item]);
                const [stockResult] = await connection.query(binary
                    ? 'UPDATE productos SET stock = stock - ? WHERE id = UUID_TO_BIN(?) AND stock >= ?'
                    : 'UPDATE productos SET stock = stock - ? WHERE id = ? AND stock >= ?', [item.quantity, item.product_id, item.quantity]);
                if (stockResult?.affectedRows === 0) {
                    throw new Error('Stock insuficiente para completar la orden');
                }
            }
            // Historial: estado inicial
            await connection.query(`INSERT INTO historial_pedido (id, orden_id, estado_anterior, estado_nuevo, observacion)
                 VALUES (${binary ? 'UUID_TO_BIN(?)' : '?'}, ${binary ? 'UUID_TO_BIN(?)' : '?'}, NULL, ?, ?)`, [(0, uuid_1.v4)(), orderId, initialEstado, 'Pedido creado. Esperando confirmación de pago.']);
    ...
    static async markCartSessionConverted(sessionId, orderId) {
        const binary = await detectIdType();
        await database_1.pool.query(`UPDATE cartsessions SET status = 'CONVERTED', order_id = ${binary ? 'UUID_TO_BIN(?)' : '?'}, updated_at = NOW() WHERE session_id = ?`, [orderId, sessionId]);
    }
    // ── Historial de estados ──────────────────────────────────────────────────
    /** Registra un cambio en el historial del pedido */
    static async addHistorial(ordenId, estadoAnterior, estadoNuevo, adminId, observacion) {
        const binary = await detectIdType();
        const idExpr = binary ? 'UUID_TO_BIN(?)' : '?';
        await database_1.pool.query(`INSERT INTO historial_pedido (id, orden_id, estado_anterior, estado_nuevo, admin_id, observacion)
    ...
    static async getHistorial(ordenId) {
        const { expr, val } = await idToSql(ordenId);
        const [rows] = await database_1.pool.query(`SELECT h.estado_anterior, h.estado_nuevo, h.cambio_en, h.observacion,
             ...
             ORDER BY h.cambio_en ASC`, [val]);
        return rows || [];
    }
    // ── Envíos ────────────────────────────────────────────────────────────────
    static async registerShipping(data) {
        const binary = await detectIdType();
        const idExpr = binary ? 'UUID_TO_BIN(?)' : '?';
        const envioId = (0, uuid_1.v4)();
        await database_1.pool.query(`INSERT INTO envios (id, orden_id, transportadora, numero_guia, fecha_envio, link_rastreo, observacion, admin_id)
    ...
    static async getShipping(ordenId) {
        const [rows] = await database_1.pool.query(`SELECT transportadora, numero_guia, fecha_envio, link_rastreo, observacion FROM envios WHERE orden_id = ? LIMIT 1`, [ordenId]);
        return rows?.[0] || null;
    }
    // ── Actualizar estado con validación de transición ────────────────────────
    static async updateOrderStatus(orderId, estado, adminId) {
        ...
        if (estado === 'ENVIADO') {
            ...
            if (!envio)
                throw new Error('Debe registrar la guía de envío antes de marcar como ENVIADO');
        }
        const { expr, val } = await idToSql(orderId);
        await database_1.pool.query(`UPDATE ordenes SET estado = ?, actualizado_en = NOW() WHERE id = ${expr}`, [estado, val]);
    ...
    static async updateTransactionCode(orderId, transactionCode) {
        const { expr, val } = await idToSql(orderId);
        await database_1.pool.query(`UPDATE ordenes SET codigo_transaccion = ?, actualizado_en = NOW() WHERE id = ${expr}`, [transactionCode, val]);
    }
    static async updatePaymentInfo(orderId, data) {
        ...
        await database_1.pool.query(`UPDATE ordenes SET ${sets.join(', ')} WHERE id = ${expr}`, [...params, val]);
    }
    static async reinstateCancelledOrderPaid(orderId) {
        const connection = await database_1.pool.getConnection();
        const binary = await detectIdType();
        const idExpr = binary ? 'UUID_TO_BIN(?)' : '?';
        ...
        try {
            await connection.query('BEGIN');
            const [resOrder] = await connection.query(`SELECT estado FROM ordenes WHERE id = ${idExpr} FOR UPDATE`, [orderId]);
            const current = String(resOrder?.[0]?.estado || '').toUpperCase();
            if (current !== 'CANCELADO') {
                await connection.query('COMMIT');
                return;
            }
            const [resItems] = await connection.query(`SELECT ${productoIdRead} AS producto_id, cantidad FROM detalleordenes WHERE orden_id = ${idExpr}`, [orderId]);
            for (const it of (resItems || [])) {
                ...
                const [stockResult] = await connection.query(binary
                    ? 'UPDATE productos SET stock = stock - ? WHERE id = UUID_TO_BIN(?) AND stock >= ?'
                    : 'UPDATE productos SET stock = stock - ? WHERE id = ? AND stock >= ?', [qty, pid, qty]);
                if (stockResult?.affectedRows === 0) {
                    throw new Error('Stock insuficiente para reactivar el pedido pagado');
                }
            }
            await connection.query(`UPDATE ordenes SET estado = ?, actualizado_en = NOW() WHERE id = ${idExpr}`, ['PAGADO', orderId]);
            await connection.query(`INSERT INTO historial_pedido (id, orden_id, estado_anterior, estado_nuevo, observacion)
                 VALUES (${idExpr}, ${idExpr}, ?, ?, ?)`, [(0, uuid_1.v4)(), orderId, 'CANCELADO', 'PAGADO', 'Pago aprobado en Wompi (reconciliado)']);
            await connection.query('COMMIT');
        }
        catch (e) {
            await connection.query('ROLLBACK');
    ...
    static async cancelAndRestock(orderId, observacion) {
        const connection = await database_1.pool.getConnection();
        const binary = await detectIdType();
        const idExpr = binary ? 'UUID_TO_BIN(?)' : '?';
        const productoIdRead = binary ? 'BIN_TO_UUID(producto_id)' : 'producto_id';
        try {
            await connection.query('BEGIN');
            const [resOrder] = await connection.query(`SELECT estado FROM ordenes WHERE id = ${idExpr} FOR UPDATE`, [orderId]);
            const current = resOrder?.[0]?.estado;
            if (String(current || '').toUpperCase() === 'CANCELADO') {
                await connection.query('COMMIT');
                return;
            }
            await connection.query(`UPDATE ordenes SET estado = ?, actualizado_en = NOW() WHERE id = ${idExpr}`, ['CANCELADO', orderId]);
            // Historial (si existe tabla): admin_id NULL
            try {
                await connection.query(`INSERT INTO historial_pedido (id, orden_id, estado_anterior, estado_nuevo, observacion)
                     VALUES (${idExpr}, ${idExpr}, ?, ?, ?)`, [(0, uuid_1.v4)(), orderId, String(current || '').toUpperCase() || null, 'CANCELADO', String(observacion || 'Pedido cancelado').trim()]);
            }
            catch {
                // ignore
            }
            ...
            for (const it of (resItems || [])) {
                ...
                if (!pid || !Number.isFinite(qty) || qty <= 0)
                    continue;
                await connection.query(binary ? 'UPDATE productos SET stock = stock + ? WHERE id = UUID_TO_BIN(?)'
    ...
    static async getAdminOrderById(orderId) {
        ...
        const [rows] = await database_1.pool.query(`SELECT 
            ...
            GROUP BY o.id, o.total, o.estado, o.direccion_envio, o.codigo_transaccion, o.creado_en,
                     ...
                     ${extraSelect ? `, ${extraSelect}` : ''}`, [orderId]);
    ...
    static async cancelExpiredOrders() {
        ...
        const [rows] = await database_1.pool.query(`SELECT ${idRead} AS id FROM ordenes 
