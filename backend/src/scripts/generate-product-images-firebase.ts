...
const STATE_FILE = path.join(STATE_DIR, 'product-image-generation-state.json');
const REPORT_FILE = path.join(STATE_DIR, 'product-image-generation-report.json');
...
const hasAnyImage = (p: CatalogProduct): boolean => {
...
const hashSeed = (raw: string): number => {
    let h = 2166136261;
...
const fetchCatalog = async (limit: number): Promise<CatalogProduct[]> => {
    const pageSize = 200;
    ...
    while (products.length < limit) {
        ...
        for (const item of items) {
            products.push(item as CatalogProduct);
...
const buildPrompt = (p: CatalogProduct): string => {
...
const generateImageFromPrompt = async (p: CatalogProduct): Promise<Buffer> => {
    ...
    if (!buf.length) throw new Error('Image generation returned empty buffer');
    return buf;
};
...
const uploadPngToFirebase = async (product: CatalogProduct, pngBuffer: Buffer): Promise<string> => {
    if (!bucket) {
...
const parseRateLimitResetSeconds = (res: Response): number | null => {
...
const updateProductImage = async (product: CatalogProduct, pngBuffer: Buffer): Promise<void> => {
...
const withRetries = async <T>(fn: () => Promise<T>, label: string, retries = 3): Promise<T> => {
    let lastError: any;
...
const loadState = async (): Promise<JobState> => {
    await fs.mkdir(STATE_DIR, { recursive: true });
    try {
        const raw = await fs.readFile(STATE_FILE, 'utf8');
        const parsed = JSON.parse(raw) as JobState;
        if (!parsed || typeof parsed !== 'object' || !parsed.items) throw new Error('Invalid state file');
        return parsed;
    } catch {
...
const saveState = async (state: JobState): Promise<void> => {
