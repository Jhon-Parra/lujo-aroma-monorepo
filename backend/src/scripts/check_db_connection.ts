import { mysqlPool, pool } from '../config/database';
...
async function runDiagnostic() {
    ...
    try {
        ...
        const [serverInfo]: any = await connection.query('SELECT DATABASE() as db, VERSION() as version, USER() as user');
        ...
        for (const table of tables) {
            try {
                const [countResult]: any = await connection.query(`SELECT COUNT(*) as total FROM ${table.name}`);
        ...
        try {
            const [products]: any = await connection.query('SELECT nombre, precio, stock FROM productos ORDER BY creado_en DESC LIMIT 3');
