...
const getSitemap = async (req, res) => {
    try {
        ...
        let products = [];
        try {
            const [rows] = await database_1.pool.query(`SELECT nombre, creado_en FROM productos WHERE activo = 1`);
            products = rows;
        }
        catch {
            // Fallback si la columna 'activo' no existe
            const [rows] = await database_1.pool.query(`SELECT nombre, creado_en FROM productos`);
            products = rows;
        }
        if (Array.isArray(products)) {
            products.forEach((p) => {
                if (!p.nombre)
                    return;
                const slug = (0, slug_util_1.generateSlug)(p.nombre);
        ...
        try {
            const [categories] = await database_1.pool.query(`SELECT nombre FROM categorias`);
            if (Array.isArray(categories)) {
                categories.forEach((c) => {
                    if (!c.nombre)
                        return;
                    const slug = (0, slug_util_1.generateSlug)(c.nombre);
                    xml += `  <url>\n`;
