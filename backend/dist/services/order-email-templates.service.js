...
exports.OrderEmailTemplateService = exports.ORDER_EMAIL_STATUSES = void 0;
const database_1 = require("../config/database");
exports.ORDER_EMAIL_STATUSES = [
    'PAGADO',
    'ENVIADO',
    'ENTREGADO',
    'CANCELADO'
        ...
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="border-collapse:separate;width:600px;max-width:600px;background:#ffffff;border:1px solid #efe8d8;border-radius:18px;overflow:hidden;">
                ...
                <tr>
                  <td align="left" style="vertical-align:middle;">
                    <a href="{{store_url}}" style="text-decoration:none;display:inline-block;">
                      <img src="{{logo_url}}" alt="Lujo&Aroma | Perfumes Bogotá" width="150" style="display:block;border:0;outline:none;text-decoration:none;height:auto;max-width:150px;" />
                    </a>
                  </td>
          ...
          <tr>
            <td style="background:#fafafa;padding:14px 22px;text-align:center;font-family:Arial,sans-serif;color:#7a7468;font-size:12px;">
              © {{year}} Lujo&Aroma | Perfumes Bogotá · <a href="{{store_url}}" style="color:#7a7468;text-decoration:underline;">perfumesbogota.com</a>
            </td>
          </tr>
        </table>
      </td>
...
`;
const DEFAULT_TEMPLATES = exports.ORDER_EMAIL_STATUSES.reduce((acc, status) => {
...
const normalizeStatus = (value) => {
    ...
    return exports.ORDER_EMAIL_STATUSES.includes(status) ? status : null;
};
exports.OrderEmailTemplateService = {
    normalizeStatus,
    getDefaultTemplate(status) {
    ...
    async listTemplates() {
        const [rows] = await database_1.pool.query(`SELECT status, subject, body_html, body_text
             FROM orderemailtemplates`);
        const map = new Map((rows || []).map((r) => [String(r.status || '').toUpperCase(), r]));
        return exports.ORDER_EMAIL_STATUSES.map((status) => {
            const row = map.get(status);
            if (!row)
                return this.getDefaultTemplate(status);
            return {
                status,
    ...
    async upsertTemplate(status, input) {
        ...
        await database_1.pool.query(`INSERT INTO orderemailtemplates (status, subject, body_html, body_text, updated_at)
             VALUES (?, ?, ?, ?, NOW())
             ON DUPLICATE KEY UPDATE subject = VALUES(subject), body_html = VALUES(body_html), body_text = VALUES(body_text), updated_at = NOW()`, [status, subject, body_html, body_text]);
        return {
            status,
            subject,
            body_html,
